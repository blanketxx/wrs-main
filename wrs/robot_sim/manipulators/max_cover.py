...
def cover_points_with_ortools(apple_positions, sphere_radius):
    ...
    if not solver:
        print("SCIP solver不可用.")
        return

    # 初始化变量
    num_apples = apple_positions.shape[0]
    sphere_centers = []      # 记录球体的候选中心
    selected_spheres = []    # 记录是否选择球体

    # 构建球体候选中心，每个中心直接位于点之上，Z坐标固定为球体半径以贴近XY平面
